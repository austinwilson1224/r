...
fancy = function(strategy = simple_strategy(even), quants = .9, nplayers = 10, nplayers_plot = 10, ntimes = 100, print = TRUE) {
  # insert trash here ....
  d = data.frame(play(strategy = strategy, nplayers = nplayers, ntimes = ntimes))
  # d$time =  seq(ntimes)
  time = seq(ntimes)
  # g = ggplot()
  ...
  g = g + geom_line(aes(time, X1), data = d, alpha=.2)
  # plot(g)
  
  for(x in colnames(d)) {
    print(x)
    g = g + geom_line(aes(time, d[,x]), alpha=.2)
  }
...
# plot(test)
plot(test$X1, type = "l")
test$row_means = rowMeans(test)
plot(test$row_means, type="l")
test$row_medians = apply(test, 1, median)




test$one = apply(test[,1:10], 1, quantile, probs = c(.1))
test$ninety = apply(test[,1:10], 1, quantile, probs = c(.9))

quantile(test,probs = c(1))


quantile(test[1,1:10], probs = c(.1))
quantile(test[1,1:10], probs = c(.9))
...
test$nintey = apply(test, 1, quantile(c(1)))
```
