...
grep("Interest", columns, value = TRUE)

# "TIC.Interest.Rate"                "NIC.Interest.Rate"               
# [28] "Interest.Type"                    "Other.Interest.Type" 
```
...
get_lease = function(data = data, column) {
  grepl(pattern = "lease", x = data[,column])
}




get_lease(data, "Financial.Advisor")
...
colnames(data)
dim(data)
sadf = sapply(data, grepl, pattern = "lease", value = TRUE)

x1 = data[1,]

grep("lease", x1)

sapply(x = data, FUN = grep("lease"))
l


grep(x = data$CDIAC.Number, "lease")
...
colnames(data)
# TIC.Interest.Rate"                "NIC.Interest.Rate"  
model1 = lm(TIC.Interest.Rate ~ Principal.Amount + New.Money, data = data)
model2 = lm(TIC.Interest.Rate ~ Principal.Amount + New.Money + Refunding.Amount + Net.Issue.Discount.Premium, data = data)
model3 = rpart(TIC.Interest.Rate ~ Principal.Amount + New.Money + Refunding.Amount + Net.Issue.Discount.Premium, data = data)
